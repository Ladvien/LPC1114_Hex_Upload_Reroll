...
unsigned char arrayForFileChar[MAX_SIZE];
...
void encode()
{
	...
	unsigned char UUEBuff[MAX_SIZE];
	int intConversion = 0;
	...
	while(UUEIndex < testSize){
		//if > 4 bytes -- handles last few bytes.
//////////////////////////////ONE//////////////////////////////////////			//                        1    1
		//Load the first byte [000000]00
		UUEBuff[UUEIndex] = testUUE_HEX[UUEIndex];
		//Make it senary.
		UUE_Chop[0] = testUUE_HEX[UUEIndex];  //Load byte one for chopping.
		UUEBuff[UUEIndex] >>= 2;  //Clip right two-bits.
		intConversion = UUEBuff[UUEIndex];
		intConversion = intConversion + 32;
		UUEBuff[UUEIndex] = intConversion;
		
		printf(" 0x%2x", (UUEBuff[UUEIndex]));
...
/////////////////////////////TWO////////////////////////////////////////	
		...
		UUEBuff[UUEIndex] = testUUE_HEX[UUEIndex]; //Load second byte.
		//Make it senary.
		UUE_Chop[1] = testUUE_HEX[UUEIndex];  //Load byte two for chopping.
...
/////////////////////////THREE////////////////////////////////////////////	
		...
		//Crashing BELOW
		UUEBuff[UUEIndex] = testUUE_HEX[UUEIndex]; //Load third byte.
		//Make it senary.
		UUE_Chop[0] = testUUE_HEX[UUEIndex];  //Load byte three for chopping.
		UUEBuff[UUEIndex] >>= 6; //Get two bits from third byte.
		UUE_Seam[1] = UUE_Chop[1]; //Load chop two into seam.
		//1111[11110000]
		UUE_Seam[1] <<= 4; //Get chop two into place.
		//[00111100]00
		UUE_Seam[1] >>= 2; //Get chop two into place.
		UUEBuff[UUEIndex] |= UUE_Seam[1]; //[00 0000]0000
		intConversion = UUEBuff[UUEIndex];
		intConversion = intConversion + 32;
		UUEBuff[UUEIndex] = intConversion;
